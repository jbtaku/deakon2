...
import { favorite, history } from '@/const/nav'
...
function Favorites() {
    ...
    useEffect(() => {
        if (allEvents && allEvents?.length !== 0 && id && token) {
            axios.post("/api/get-favorites.php", { id: id, token: token }).then((res) => {
    ...
    return (
        <div>
            <Headline img='/images/party/image.jpg' title='お気に入り' titleEn='FAVORITE' breadcrumbs={[favorite]} />
